...
**Textures (fugal accompaniment)**: `CompositionConfiguration.TextureConfiguration` (param #21, default None; the Texture select shown under the Fugue form; requires `VoiceRhythmConfiguration.Enabled` and ≥2 voices) trades the fugal body's imitative fabric for melody-over-accompaniment. The `VoiceRhythmScheduler`'s texture mode supersedes its per-block rotation (both rotating answers decline) with one static register-derived assignment — highest voice = MELODY (florid store), lowest = FIGURATION (its own 4th ledger store), middles = PADS (held store + their own 5th pad store; ties are opportunistic, no walk pins; under the figural textures the pads BREATHE — `PadGentleFigures` = {PassingTone, NeighborTone} at weight 20, everything else silenced; a figured pad breaks its tie FORWARD only, while a preceding repeat still ties INTO it, silencing the principal under the sounding gentle sub-note, and the suspension pass may overwrite a breath outright; user-disabling both gentle figures silences breathing entirely; Chordal's pads stay fully silent, its identity being the figure-free accompaniment; the pad store is what separates pads from the held store's other tenants, the rotation's held voice and the ground's tread, which never breathe) — ordered by MinNote desc, MaxNote desc, then Instrument. `VoiceRhythmPolicyTransformer` derives per-processor texture weights at engine build (in-family 95 / out-of-family 0 / None = stock; families are onset-spacing-uniform: Walking = {RepeatedNote, PassingTone} quarter tread, BrokenChord = {Arpeggio (the degree-gated chord-tone cell carrying the fabric) + the four moving-interior pedal figures + Pedal} eighth pattern — the four moving pedal figures ride a very-low accent tier (`BrokenChordAccentFigures`, weight 10, a subset only modulating weight within the spacing family) so Arpeggio + Pedal carry, and the static-interior octave pedals are deliberately evicted (their cell is one pitch class four times; a census showed that bounce claiming ~40% of the fabric; the whole six-figure octave-pedal family also sits far below the stock workhorse tier — 25 moving / 15 static — the ear reads the recurring bounce as a mannerism), Chordal = empty — user-disabling a family's figures degrades that texture toward Chordal), and the gate chain resolves held → texture (short-circuit, NEVER ×intensity) → florid → ×intensity, one draw per item. The body walk records roles per beat, and after phrasing the composer re-records the phraser's deep-copied restatements (idempotent on the reference-keyed stores): an accompaniment copy sheds its copied-in figures first (theme-phrase restatements carry the exposition's stock decoration by value) so the second decoration pass re-clothes it in-family, while the melody keeps its copied figures. Under an active texture the whole-composition ornamentation pass decorates in the structural hierarchy — melody, then figuration, then pads — so the cleaners' just-decorated-loses rule sacrifices the less structural line within each pass (the melody wins everything; the fabric wins over a pad's breath); the sustain pass keeps raw order (its gate draws per item — reordering would shift the shared stream), and `EndingComposer`'s whole-composition calls re-order harmlessly (no recorded notes). After dynamics, `Composer.ApplyTextureProminence` offsets accompaniment velocities by −8 (clamped to the instrument window, sub-notes mirrored, exactly once, draw-free, gated on the scheduler so rotation-held notes are never offset). The exposition and ending stay stock; a ground render that produces a plan never records texture; the unplannable-ground fallback fugue TAKES the texture (it is a fugue — the pattern popover documents this since the Texture control is hidden under the ground form); texture-off (None / voice-rhythm-off / <2 voices) renders byte-identical to pre-texture (decided by a cross-branch hash harness at ship time, held since by the scheduler's decline pins and the empty-store weight identity). Legacy saves load as None; Randomize preserves the texture, Reset clears it.
...
- **Architecture rules**: structural conventions are enforced by `tests/BaroquenMelody.Architecture.Tests` — see [Architecture Tests](#architecture-tests) before adding types, namespaces or fixtures.
...
- **Assemblies**: Infrastructure depends on neither the Library nor the UI; the Library, console app and benchmarks never depend on the UI; only the UI may reference MudBlazor and `Fluxor.Blazor.*`; the UI never references `Microsoft.Maui.*`.
...
- Compare `SeededComposition.Notes` lists with the ordered `Should().Equal(...)` / `Should().NotEqual(...)` only, never `BeEquivalentTo` / `NotBeEquivalentTo`: the snapshots are already in `GetNotes()` time order, and FluentAssertions' unordered structural matching is quadratic — measured at ~20 s per ~1,000-note list against ~1 s for the two compositions being compared (four such assertions were 59 % of the suite's CPU time before 2026-09-04).
- `Library.Tests` runs its fixtures in parallel (`[assembly: Parallelizable(ParallelScope.Fixtures)]` in `AssemblyInfo.cs`; tests within a fixture stay sequential). Keep fixtures free of shared mutable state, or mark one `[NonParallelizable]`. CI collects coverage with coverlet's `SingleHit=true` (identical line/branch coverage, ~4× cheaper on the composition loops) and builds Release.
