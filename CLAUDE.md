...
- **Architecture rules**: `tests/BaroquenMelody.Architecture.Tests` (ArchUnitNET, NUnit) gates assembly boundaries, namespace residency, type shapes (sealed/record/visibility), forbidden dependencies, and test-suite conventions; it runs in CI like the other test projects. When a rule fails, prefer fixing the code; extend an allowlist only with a `Because()` that explains the exception. Never hand-edit `FrozenViolations/*.json` — a frozen rule's baseline is regenerated by running the tests and must be committed deliberately.
