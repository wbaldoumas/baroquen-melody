...
- One fixture per tier: `LayerDependencyTests` (A-1..A-8 assembly boundaries), `LibraryStructureTests` (L-1..L-18), `FrontendStructureTests` (F-1..F-3), `TestSuiteConventionTests` (T-1..T-4 plus the T-4 baseline guard), `CompositionShardTests` (the CI shard map in `.github/workflows/composition-shards.json` partitions the `[Category("Composition")]` fixtures exactly and every shard has a matrix entry in `test.yml`; plain reflection + JSON, located through `[CallerFilePath]` like the frozen-violations store), `MutationShardTests` (the mutation shard map in `.github/workflows/mutation-shards.json` partitions the Library's top-level source folders exactly and every shard is listed under `shard:` in `mutation.yml`), `PositiveControlTests` (rules that MUST fail — they prove each detection mechanism can fire).
