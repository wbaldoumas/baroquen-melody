...
- CI runs the Composition fixtures in shards: `.github/workflows/composition-shards.json` maps each shard to fixture full names, `test.yml` turns a shard's list into `FullyQualifiedName~` filters, and `CompositionShardTests` in the Architecture suite fails when a tagged fixture is missing from the map, listed twice, or a shard has no matrix entry. A new Composition fixture therefore goes into one shard — the lightest by runner time — in the same PR, tagged at the fixture (the guard rejects method-level `Composition` tags: the unit leg would exclude such a test and no shard would select it) and kept plain (not parameterized, sourced, generic or nested — the name filter would not match). The library step runs with `RunConfiguration.TreatNoTestsAsError=true`, so a shard whose names stop matching fails instead of passing empty.
