...
- Every seeded composition sweep is CI time: the 13 `[Category(TestCategories.Composition)]` fixtures are ~94 % of the suite. Keep `N` small, keep the fixture tagged `Composition` (Stryker's `test-case-filter` excludes the category; the NUnit adapter honours category filters and silently drops any other filter selecting > 2,000 tests), and read the per-test durations from the `test-results-*` artifacts before adding a sweep to a fixture that is already the longest.
- Categories are the `TestCategories` constants, never literals: `MutationUniverseTests` in the Architecture suite requires the suite to tag exactly the declared names and Stryker's filter to exclude exactly them. A test that composes a whole piece without being a sweep — a determinism pin, a configure-and-compose smoke test — is tagged `TestCategories.WholeComposition` at the method (at the fixture only when every test in it composes): CI's unit leg still runs it, Stryker does not. Such a test covers nearly every mutant, so it ran against nearly every one; the tagged tests were about 70 % of a mutant's test time and caught 5 of 1,657 kills the fast unit tests missed. Keep one fast, untagged representative wherever a whole-piece test is a path's only coverage (the configurator keeps six (mode, meter) pairs of its matrix untagged); a property that must hold under mutation belongs in a unit test of the component, not in a tagged composition.
